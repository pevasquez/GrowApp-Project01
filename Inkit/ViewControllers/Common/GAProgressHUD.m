...
- (void)setMode:(GAProgressHUDMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    if (mode == GAProgressHUDModeDeterminate) {
        if ([NSThread isMainThread]) {
            [self updateProgress];
            [self setNeedsDisplay];
        } else {
...
- (void)updateProgress {
    [(GARoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == GAProgressHUDModeDeterminate) {
        self.indicator = [[GARoundProgressView alloc] init];
    }
    else if (mode == GAProgressHUDModeCustomView && self.customView != nil){
...
+ (GAProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    GAProgressHUD *hud = [[GAProgressHUD alloc] initWithView:view];
    [view addSubview:hud];
    [hud show:animated];
    return hud;
}
...
- (id)initWithView:(UIView *)view {
    ...
    if (!view) {
        [NSException raise:@"GAProgressHUDViewIsNillException"
                    format:@"The view used in the GAProgressHUD initializer is nil."];
    }
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.mode = GAProgressHUDModeIndeterminate;
        self.labelText = nil;
...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) {
        ...
        if (nil != self.detailsLabelText) {
            // Get size of label text
            dims = [self.detailsLabelText sizeWithAttributes:@{NSFontAttributeName: self.detailsLabelText}];
...
@implementation GARoundProgressView
